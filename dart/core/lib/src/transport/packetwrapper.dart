...
abstract class PacketWrapper<T> extends JsonSerializableClass {
  ...
  @override
  String toString() {
    return ", aClass='" + packetIdentifier + '\'' +
        '}' +
        'PacketWrapper{';
  }
...
@JsonSerializable(explicitToJson: true, createFactory: false)
class ImplPacketWrapper extends PacketWrapper<dynamic> implements AcceptablePacketTypes {

  static final ImplPacketWrapper _constant = ImplPacketWrapper._self();
  ImplPacketWrapper._self() : super.empty();

  ImplPacketWrapper.fromString(bool encrypt, String packetIdentifier) : super.empty(null, packetIdentifier) {
...
//    print("JSON Deocded of UnencryptedPacketWrapper = ${json[PacketWrapper.getPacketIdentifier()]}\n object: ${json[PacketWrapper.getJsonIdentifier()]}");
    return ImplPacketWrapper.fromString(json[PacketWrapper.getEncryptIdentifier()], json[PacketWrapper.getPacketIdentifier()]);
  }

  /// A necessary factory constructor for creating a new User instance
  /// from a map. Pass the map to the generated `_$UserFromJson()` constructor.
  /// The constructor is named after the source class, in this case, User.
  factory ImplPacketWrapper.fromJson(Map<String, dynamic> json) => _constant.fromJson(json);
...
class UnencryptedPacketWrapper extends PacketWrapper<Map<String, dynamic>> implements AcceptablePacketTypes {
  ...
  static final UnencryptedPacketWrapper _constant = UnencryptedPacketWrapper._self();
  ...
  UnencryptedPacketWrapper.fromString(Map<String, dynamic> json, String packetIdentifier) : super(JsonSerializableMap(json), packetIdentifier) {
    if (PacketRegistry.checkIfRegisteredIdentifier(packetIdentifier) == RegisteredReturnValues.NOT_IN_REGISTRY) {
      throw ('The packet trying to be wrapped is not registered. \"$packetIdentifier\"');
    }
...
class EncryptedPacketWrapper extends PacketWrapper<EncryptedBytes> implements AcceptablePacketTypes {
  ...
  static final EncryptedPacketWrapper _constant = EncryptedPacketWrapper._self();
...
class EncryptedBytes implements JsonSerializableClass {
  ...
  Uint8List toUint8List(List<dynamic> list) {
    var intList = list.map((f) =>
    f.runtimeType == String // f is a string
        ? int.tryParse(f) == null /// If null, f is not a string
