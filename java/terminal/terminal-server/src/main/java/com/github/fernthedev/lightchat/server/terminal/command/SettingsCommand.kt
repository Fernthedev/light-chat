...
class SettingsCommand(private val server: Server) : Command("settings") {
    override fun onCommand(sender: SenderInterface, args: Array<String>) {
        if (args.isEmpty()) {
            ...
            server.authenticationManager.authenticate(sender).thenAccept { authenticated: Boolean ->
                if (authenticated) {
                    val timeStart: Long
                    val timeEnd: Long
                    ...
                    when (arg.lowercase(Locale.getDefault())) {
                        "set" -> if (args.size > 2) {
                            ...
                            try {
                                ...
                                ServerTerminal.sendMessage(
                                    sender,
                                    ColorCode.GREEN.toString() + "Set " + settingName + " to " + newValue
                                )
                                ServerTerminal.sendMessage(
                                    sender,
                        ...
                        "list" -> {
                            ServerTerminal.sendMessage(
                                sender,
                                "Possible setting names : {possible values, empty if any are allowed}"
                            )
                            try {
